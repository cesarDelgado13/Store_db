    ...
    ***************************************
    ...
    def read_a_zip(self, zip):
        try:
            ...
            vals = (zip,)
            self.cursor.execute(sql, vals)
            record = self.cursor.fetchone() #recuperar un registro
            return record
        except connector.Error as err:
            return err
    ...
    def read_all_zips(self):
        try:
            ...
            records = self.cursor.fetchall() #recuperar todos los registros
            return records
        except connector.Error as err:
            return err
    ...
    def delete_zip(self,zip):
        try:
            sql = 'DELETE from zips where zip = %s' #cambiar Select
            vals = (zip,)
            self.cursor.execute(sql,vals)
            self.cnx.commit()
    ...
    #Product methods
    def create_product(self, name, brand, descrip, price): #quite la t
        try:
            sql = 'INSERT into product (`p_name`, `p_brand`,`p_descrip`,`p_price`) values (%s,%s,%s,%s)'
            vals = (name, brand, descrip, price) #quite la t
            self.cursor.execute(sql, vals)
            self.cnx.commit()
            return True
        except connector.Error as err:
    ...
    def read_product_brand(self, brand):
        try:
            sql = 'SELECT * from product where p_brand = %s' #cambie brand por p_brand
            vals = (brand,)
            self.cursor.execute(sql, vals)
    ...
    def read_all_products(self): #cuidado con la longitud de los datos
        try:
    ...
    def read_all_clients(self): #cuidado  con la cantidad de datos
        try:
    ...
    def create_order(self, id_client, status, date, total):
        try:
            ...
            self.cursor.execute(sql,vals)
            self.cnx.commit()
            id_order = self.cursor.lastrowid #ultimo identificador que se modifico
            return id_order
        except connector.Error as err:
            self.cnx.rollback()
            return err
    ...
    def read_all_orders(self): #Cuidado con la cantidad de datos
        try:
    ...
    def read_orders_date(self, date):
        try:                        #agregue la s en client y borre la s en id_client
